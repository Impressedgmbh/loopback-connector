...
SQLConnector.prototype.buildInsert = function(model, data, options) {
  ...
  for (const [propIndex, prop] of fields.properties.entries()) {
    const val = this._patchArrayValue(prop, insertStmt.params[propIndex]);
    if (val != undefined) {
      insertStmt.params[propIndex] = val;
    }
  }
...
SQLConnector.prototype._buildFieldsForKeys = function(model, data, keys, excludeIds) {
  ...
  for (let i = 0, n = keys.length; i < n; i++) {
    ...
    const v = this._patchArrayValue(p, data[key], true);
...
/**
 * Make params of type array PostgreSQL compliant.
 * @param {Object} p The property object.
 * @param {String} value The property's raw value.
 * @returns {String} The patched array as a string.
 * @private
 */
SQLConnector.prototype._patchArrayValue = function(p, data, convert = false) {
  let v;
  if (
    Array.isArray(p.type) &&
    p.postgresql && p.postgresql.dataType && typeof p.postgresql.dataType === 'string' &&
    /(jsonb?|uuid)\[\]$/.test(p.postgresql.dataType) &&
    data && data.length > 1 &&
    ...
    if (json.length) {
      if (typeof json[0] !== 'string') {
        arrayStr = `${json.map((item) => `"${JSON.stringify(item).replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`).join(',')}`;
      } else {
        arrayStr = `${json.map((item) => `"${item}"`).join(',')}`;
      }

      v = `{${arrayStr}}`;

      console.log(`PATCHED ARRAY VALUE: ${v}`);
    }
  } else {
    if (convert) {
      v = this.toColumnValue(p, data);
    }
  }
  return v;
};
