...
SQLConnector.prototype.buildInsert = function(model, data, options) {
  ...
  let patched = false;
  // if (model === 'Task' || model === 'Workflow') {
  for (const [propIndex, prop] of fields.properties.entries()) {
    if (Array.isArray(prop.type) && prop.postgresql?.dataType === 'jsonb[]' && insertStmt.params[propIndex]?.length > 1 && insertStmt.params[propIndex].slice(0, 1) === '[' && insertStmt.params[propIndex].slice(-1) === ']' ) {
      // Property type and property value are arraya
      if (fields.columnValues[propIndex].sql === '?') {
        fields.columnValues[propIndex].sql += '::JSONB[]'
      }

      const json = JSON.parse(insertStmt.params[propIndex])
      const arrayStr = `${json.map((item) => `"${JSON.stringify(item).replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`).join(',')}`

      insertStmt.params[propIndex] = `{${arrayStr}}`;
  ...
  if (patched) {
    console.log(`PATCHED INSERT STATEMENT: ${JSON.stringify(insertStmt)}`)
  }
  // }
...
SQLConnector.prototype._buildFieldsForKeys = function(model, data, keys, excludeIds) {
  ...
  for (let i = 0, n = keys.length; i < n; i++) {
    ...
    let v;
    if (Array.isArray(p.type) && p.postgresql?.dataType === 'jsonb[]' && data[key] && data[key].slice(0, 1) === '[' && data[key].slice(-1) === ']' ) {
      // Property type and value are array
      const json = JSON.parse(data[key]);
      const arrayStr = `${json.map((item) => `"${JSON.stringify(item).replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`).join(',')}`

      v = `{${arrayStr}}`;

      console.log(`PATCHED UPDATE VALUE: ${v}`)
    } else {
      v = this.toColumnValue(p, data[key]);
    }
    if (v !== undefined) {
      fields.names.push(k);
